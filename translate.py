...
from utils.config import get_config, latest_weights_file_path 
...
def fix_state_dict(state_dict):
    ...
    for key, value in state_dict['model_state_dict'].items():
        new_key = key.replace('residual_connections', 'residuals_connection')\
...
def translate(sentence: str):
    ...
    # Load the pretrained weights with the fix
    model_filename = latest_weights_file_path(config)
    state = torch.load(model_filename, map_location=torch.device('cpu'))
    state = fix_state_dict(state)  # Add this line
